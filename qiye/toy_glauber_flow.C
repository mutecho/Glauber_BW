...
void sample_nucleus_2D(
    vector<pair<double,double>>& nuc,
    int A,
    double R,
...
void get_participants(
    vector<pair<double,double>>& A_nuc,
    ...
    // 只加入一次！
    for (int i = 0; i < NA; i++) {
        if (hitA[i]) parts.push_back(A_nuc[i]);
    }

    for (int j = 0; j < NB; j++) {
        if (hitB[j]) parts.push_back(B_nuc[j]);
    }
}
...
void compute_eccentricity(
    const vector<pair<double,double>>& parts,
    double& eps2, double& psi2
) {
