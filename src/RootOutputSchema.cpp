...
namespace blastwave::io {
  ...
  namespace {
    ...
    constexpr char kInitialGeometryModeBranchName[] = "initial_geometry_mode";
    constexpr char kEps3BranchName[] = "eps3";
    constexpr char kPsi2BranchName[] = "psi2";
    constexpr char kPsi2FreezeoutBranchName[] = "psi2_f";
    constexpr char kPsi3BranchName[] = "psi3";
    constexpr char kChi2BranchName[] = "chi2";
    constexpr char kV3BranchName[] = "v3";
    constexpr char kV2WrtPsi2BranchName[] = "v2_wrt_psi2";
    constexpr char kV3WrtPsi3BranchName[] = "v3_wrt_psi3";
    constexpr char kXCenterInitBranchName[] = "x_center_init";
    constexpr char kYCenterInitBranchName[] = "y_center_init";
    constexpr char kRRmsInitBranchName[] = "R_rms_init";
    constexpr char kGeoA2BranchName[] = "geo_a2";
    constexpr char kGeoA3BranchName[] = "geo_a3";
    constexpr char kGeoR3BranchName[] = "geo_r3";
    constexpr char kGeoSigma3BranchName[] = "geo_sigma3";
  ...
  void declareEventBranches(TTree &tree, EventBranches &branches) {
    ...
    tree.Branch(kInitialGeometryModeBranchName, &branches.initialGeometryMode, "initial_geometry_mode/I");
    tree.Branch(kEps3BranchName, &branches.eps3, "eps3/D");
    tree.Branch(kPsi2BranchName, &branches.psi2, "psi2/D");
    tree.Branch(kPsi2FreezeoutBranchName, &branches.psi2Freezeout, "psi2_f/D");
    tree.Branch(kPsi3BranchName, &branches.psi3, "psi3/D");
    tree.Branch(kChi2BranchName, &branches.chi2, "chi2/D");
    tree.Branch(kV3BranchName, &branches.v3, "v3/D");
    tree.Branch(kV2WrtPsi2BranchName, &branches.v2WrtPsi2, "v2_wrt_psi2/D");
    tree.Branch(kV3WrtPsi3BranchName, &branches.v3WrtPsi3, "v3_wrt_psi3/D");
    tree.Branch(kXCenterInitBranchName, &branches.xCenterInit, "x_center_init/D");
    tree.Branch(kYCenterInitBranchName, &branches.yCenterInit, "y_center_init/D");
  ...
  void bindEventBranches(TTree &tree, EventBranches &branches) {
    ...
    tree.SetBranchAddress(kInitialGeometryModeBranchName, &branches.initialGeometryMode);
    tree.SetBranchAddress(kEps3BranchName, &branches.eps3);
    tree.SetBranchAddress(kPsi2BranchName, &branches.psi2);
    tree.SetBranchAddress(kPsi2FreezeoutBranchName, &branches.psi2Freezeout);
    tree.SetBranchAddress(kPsi3BranchName, &branches.psi3);
    tree.SetBranchAddress(kChi2BranchName, &branches.chi2);
    tree.SetBranchAddress(kV3BranchName, &branches.v3);
    tree.SetBranchAddress(kV2WrtPsi2BranchName, &branches.v2WrtPsi2);
    tree.SetBranchAddress(kV3WrtPsi3BranchName, &branches.v3WrtPsi3);
    tree.SetBranchAddress(kXCenterInitBranchName, &branches.xCenterInit);
    tree.SetBranchAddress(kYCenterInitBranchName, &branches.yCenterInit);
