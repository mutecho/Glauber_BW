...
GeneratedEvent BlastWaveGenerator::generateEvent(int eventId) {
  ...
  for (const Nucleon& participant : participants) {
    ...
    for (int iParticle = 0; iParticle < multiplicity; ++iParticle) {
      ...
      const FlowVelocity beta =
          sampleFlowVelocity(emissionPoint, etaS, info.eps2, info.psi2);
...
BlastWaveGenerator::Nucleon BlastWaveGenerator::sampleSingleNucleon(
  ...
  while (true) {
    ...
    const double density =
        1.0 /
        (1.0 +
         std::exp((radius - config_.woodsSaxonRadius) /
                  config_.woodsSaxonDiffuseness));
...
std::pair<double, double> BlastWaveGenerator::computeParticipantShape(
  ...
  const double eps2 =
      std::sqrt(eccentricityX * eccentricityX + eccentricityY * eccentricityY) /
      denominator;
...
BlastWaveGenerator::SpatialPoint BlastWaveGenerator::smearSource(
  ...
  return {participant.x + smearDistribution(rng_),
...
double BlastWaveGenerator::sampleEtaS() {
  ...
  if (config_.etaPlateauHalfWidth > 0.0 && config_.sigmaEta <= 0.0) {
    std::uniform_real_distribution<double> plateauDistribution(
...
BlastWaveGenerator::FourMomentum BlastWaveGenerator::sampleThermalMomentum() {
  ...
  const double energy =
      std::sqrt(config_.mass * config_.mass + momentumMagnitude * momentumMagnitude);
  return {px, py, pz, energy};
}

BlastWaveGenerator::FlowVelocity BlastWaveGenerator::sampleFlowVelocity(
  ...
  const double modulation =
      1.0 + 2.0 * config_.kappa2 * eps2 * std::cos(2.0 * (phi - psi2));
...
BlastWaveGenerator::FourMomentum BlastWaveGenerator::lorentzBoost(
    const FourMomentum& localMomentum,
    const FlowVelocity& beta) const {
  const double betaSquared =
      beta.bx * beta.bx + beta.by * beta.by + beta.bz * beta.bz;
  ...
  const double betaDotP =
      beta.bx * localMomentum.px + beta.by * localMomentum.py + beta.bz * localMomentum.pz;
  const double gammaFactor = (gamma - 1.0) / betaSquared;

  FourMomentum boostedMomentum;
  boostedMomentum.px =
      localMomentum.px + gammaFactor * betaDotP * beta.bx + gamma * beta.bx * localMomentum.energy;
  boostedMomentum.py =
      localMomentum.py + gammaFactor * betaDotP * beta.by + gamma * beta.by * localMomentum.energy;
  boostedMomentum.pz =
      localMomentum.pz + gammaFactor * betaDotP * beta.bz + gamma * beta.bz * localMomentum.energy;
...
void BlastWaveGenerator::validateParticle(const ParticleRecord& particle) const {
  const double fields[] = {
      particle.mass, particle.x,     particle.y,     particle.z,     particle.t,
      particle.px,   particle.py,    particle.pz,    particle.energy, particle.etaS,
