...
- 流体元速度抽样模块位于 [include/blastwave/FlowFieldModel.h](/Users/allenzhou/Research_software/Blast_wave/include/blastwave/FlowFieldModel.h) 以及：
  ...
  当前默认速度抽样器是 participant covariance ellipse 的法向流场；并列可选的 `density-normal` 会根据 smeared participant density gradient 取法向，再在平坦区回退到协方差法向。公开参数面是 `rho0`、`rho2`、`flow-power`、`flow-velocity-sampler`、`flow-density-sigma`、`debug-flow-ellipse`。
