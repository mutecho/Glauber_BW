...
- Stage completed: internal `EventMedium` and `EmissionSite` refactor with preserved public CLI/config and ROOT schema behavior
- Artifact produced:
  - added `DensityFieldModel`, `EventMedium`, and `EmissionSampler` interfaces in the ROOT-free core
  - removed the old flow-context API names instead of keeping compatibility wrappers
  - `EventMedium` now carries `participantGeometry`, `initialDensity`, `emissionDensity`, and `emissionGeometry`
  - current density evolution remains `None`, so initial and emission-stage fields are identical
  - `sampleEmissionSites()` now owns participant-hotspot multiplicity and transverse smear, returning `EmissionSite` records consumed by the generator loop
  - `RootEventFileWriter` now serializes the generator-owned `emissionDensity` for the optional `density_normal_event_density_x-y` object instead of rebuilding density independently
  - added `test_emission_sampler` and expanded flow/density tests
  - refreshed `docs/EventMedium与发射接口设计.md`, higher-authority docs, and the `project-state/` ledger
- Current contract snapshot:
  ...
  - no new CLI/config keys were added for medium evolution or emission backend selection
  - no ROOT tree, branch, or mandatory histogram name changed
...
- If future density-field emission is added, implement a new `EmissionSamplerMode` that returns `EmissionSite` rather than special-casing `BlastWaveGenerator::generateEvent()`.
- If future density evolution is added, modify `buildEventMedium()` so it updates `emissionDensity` / `emissionGeometry` while preserving `participantGeometry` for event-summary `eps2/psi2`.
